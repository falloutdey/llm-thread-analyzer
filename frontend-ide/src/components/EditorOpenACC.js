...
function EditorOpenACC() {
  ...
  const carregarProjeto = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/openacc/projetos/${idProjeto}/completo`,
  ...
  const carregarConteudoArquivo = async (id) => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api-acc/arquivos/${id}/conteudo`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleSave = async () => {
    ...
    try {
      await axios.put(
        `http://localhost:5000/api-acc/projetos/${idProjeto}/arquivos/${idArquivo}`,
  ...
  const compileFile = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:5000/api-acc/compilar/${idProjeto}/${idArquivo}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const criarArquivo = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:5000/api-acc/arquivo/arquivos`,
        {
  ...
  const deleteArquivo = async (id) => {
    try {
      await axios.delete("http://localhost:5000/api-acc/arquivos/arquivo", {
        headers: { Authorization: `Bearer ${token}` },
        data: { id_arquivo: id },
      });
  ...
  const renomearArquivo = async () => {
    try {
      await axios.post(
        `http://localhost:5000/api-acc/alterar-nome-arquivo`,
        {
