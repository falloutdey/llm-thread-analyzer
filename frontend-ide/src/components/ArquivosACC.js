...
const ArquivosACC = () => {
    ...
    useEffect(() => {
        const fetchProjects = async () => {
            ...
            try {
                const response = await fetch(`http://localhost:5000/api/openacc/projetos/${id_estudante}`, {
                    method: 'GET',
                    headers: {
    ...
    const handleOpenProject = (idProjeto, idArquivo) => {
        navigate(`/editor-open-acc/${idProjeto}/${idArquivo}`);
    };
    ...
    const handleDeleteProject = async (idProjeto) => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/api/openacc/projetos/${idProjeto}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
    ...
    const handleUpdateProject = async (idProjeto) => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/api/openacc/projetoacc/${idProjeto}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
        ...
        <div>
            ...
            <main className="main-content container">
                <section className="welcome-section">
                    <h1>Meus Projetos OpenACC</h1>
                    <p className="subtitle">Gerencie seus projetos de programação paralela</p>
                    
                    <Link to="/criar-open-acc" className="new-project-button">
                        <FaPlus /> Novo Projeto
                    </Link>
                </section>
                ...
                <section className="projects-section">
                    {isLoading ? (
                        ...
                        <div className="empty-projects">
                            <p>Você ainda não possui projetos OpenACC</p>
                            <Link to="/criar-open-acc" className="create-project-link">
                            ...
                            {projects.map((project) => (
                                <div key={project.id_projeto} className="project-card">
                                    <div className="project-header">
                                        ...
                                        <span className="project-badge">OpenACC</span>
                                    </div>
