...
const ProjetosOpenMP = () => {
    ...
    useEffect(() => {
        const fetchProjects = async () => {
            ...
            try {
                const response = await fetch(`http://localhost:5000/api/openmp/projetos/${id_estudante}`, {
                    method: 'GET',
                    headers: {
    ...
    const handleOpenProject = (idProjeto, idArquivo) => {
        navigate(`/editor-open-mp/${idProjeto}/${idArquivo}`);
    };
    ...
    const handleDeleteProject = async (idProjeto) => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/api/openmp/projetos/${idProjeto}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
    ...
    const handleUpdateProject = async (idProjeto) => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/api/openmp/projetomp/${idProjeto}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
        ...
        <div>
            ...
            <main className="main-content container">
                <section className="welcome-section">
                    <h1>Meus Projetos OpenMP</h1>
                    <p className="subtitle">Gerencie seus projetos de programação paralela</p>
                    
                    <Link to="/criar-openmp" className="new-project-button">
                        <FaPlus /> Novo Projeto
                    </Link>
                </section>
                ...
                <section className="projects-section">
                    {isLoading ? (
                        ...
                        <div className="empty-projects">
                            <p>Você ainda não possui projetos OpenMP</p>
                            <Link to="/criar-openmp" className="create-project-link">
                                ...
                                <div key={project.id_projeto} className="project-card">
                                    <div className="project-header">
                                        {editProjectId === project.id_projeto ? (
                                            ...
                                            <h3 onClick={() => setEditProjectId(project.id_projeto)}>
                                                {project.nome_projeto}
                                            </h3>
                                        )}
                                        <span className="project-badge">OpenMP</span>
                                    </div>
