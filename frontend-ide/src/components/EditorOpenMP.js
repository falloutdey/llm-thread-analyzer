...
function EditorOpenMP() {
  ...
  const carregarProjeto = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/openmp/projetos/${idProjeto}/completo`,
  ...
  const carregarConteudoArquivo = async (id) => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/arquivos/${id}/conteudo`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleSave = async () => {
    ...
    try {
      await axios.put(
        `http://localhost:5000/api/projetos/${idProjeto}/arquivos/${idArquivo}`,
  ...
  const compileFile = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:5000/api/compilar/${idProjeto}/${idArquivo}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  try {
    const response = await axios.post(
      `http://localhost:5000/api/arquivo/arquivos`,
      {
  ...
  const deleteArquivo = async (id) => {
    try {
      await axios.delete("http://localhost:5000/api/arquivos/arquivo", {
        headers: { Authorization: `Bearer ${token}` },
        data: { id_arquivo: id },
      });
  ...
  const renomearArquivo = async () => {
    try {
      await axios.post(
        `http://localhost:5000/api/alterar-nome-arquivo`,
        {
        ...
        <div className="dialog-overlay">
          <div className="dialog">
            ...
            <input
              ...
              placeholder="nome_arquivo.c"
              autoFocus
            />
            <div className="dialog-buttons">
              <button onClick={() => setShowCreateDialog(false)}>Cancelar</button>
              <button onClick={criarArquivo}>Criar</button>
            </div>
          </div>
        </div>
      )} */}
